...
sale <- sale %>% 
  ...
  st_sf()
# rename column
colnames(sale)[50] <- 'elmtr_school'

school.high <- school.tidy %>% 
  filter(LEVEL_ == "HIGH")
...
sale <- sale %>% 
  dplyr::mutate(
    hsch_distance = st_distance(sale, school.high, by_element = TRUE)
  )
```
