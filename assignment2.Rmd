...
#              y = paste(toModel.1$quallifyCode,toModel.1$elmtr_school,toModel.1$mainfloorSF,toModel.1$HeatingDscr),
#              p = .75, list = FALSE)
...
```
### model 5 - add elmtr_distance to model 6
```{r}
...
### model 2 -add median price to model 6
```{r}
#create lagPrice
coords <- st_coordinates(sale.final) 

neighborList <- knn2nb(knearneigh(coords, 10))

spatialWeights <- nb2listw(neighborList, style="W")

sale.final.lag <- sale.final

sale.final.lag$LNlagPrice <- lag.listw(spatialWeights, sale.final.lag$LNprice)

toModel.lag <- sale.final.lag %>% 
  filter(toPredict == 0)
#inTrain.2 <- createDataPartition(
#              y = paste(toModel.lag$GEOID,toModel.lag$elmtr_school,toModel.lag$TotalFinishedSF, #toModel.lag$section_num,toModel.lag$ExtWallPrim, toModel.lag$Heating),
#              p = .75, list = FALSE)
sale.training.lag <- toModel.lag[inTrain.2,] 
sale.test.lag <- toModel.lag[-inTrain.2,]  


reg.training.2 <- lm(LNprice ~ year_quarter + bld_num + section_num + designCode + 
    qualityCodeDscr + ConstCode + builtYear + CompCode + EffectiveYear + 
    LNbsmtSF + bsmtType + carStorageSF + nbrBedRoom + nbrRoomsNobath + 
    LNmainfloorSF + LNnbrThreeQtrBaths + LNnbrFullBaths + LNnbrHalfBaths + 
    LNTotalFinishedSF + AcDscr + Heating + ExtWallPrim + ExtWallDscrSec + 
    IntWall + Roof_CoverDscr + high_school + elmtr_school + hsch_distance + 
    city + GEOID + parkCount + LNcrime_nn1 + LNcrime_nn2 + LNcrime_nn3 + 
    LNcrime_nn4 + LNcrime_nn5 + in_hist*BoulderCity + LNtrailheadCount + LNlagPrice, 
    data = st_drop_geometry(sale.training.lag))

summary(reg.training.2)
```
...
reg.test2 <-
  sale.test.lag %>%
  mutate(price.Predict = exp(predict(reg.training.2, sale.test.lag))-1,
         price.Error = price.Predict - price,
         price.AbsError = abs(price.Predict - price),
         price.APE = (abs(price.Predict - price)) / price.Predict)


mean(reg.test2$price.AbsError, na.rm = T)
mean(reg.test2$price.APE, na.rm = T)
...
toPredict <-
  ...
  dplyr::select(-price) %>% 
  cbind(
          data.frame(price = exp(predict(reg.training.2, toPredict))-1)
  ) %>% 
  dplyr::select(MUSA_ID, price)

#write.csv(toPredict,file="prediction.csv",quote=F,row.names = F)
```
