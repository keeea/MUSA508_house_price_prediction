...
school.high <- right_join(
  ...
  dplyr::select(SchoolNumber, Name, Total_Percent_Points, geometry)
# join SVV high
school.high <- rbind(
  right_join(
  school.ratings,
...
school.high.point <- school.high %>% 
  ...
  st_sf() %>% 
  dplyr::select(Name)
school.high.point <- school.high.point[st_buffer(county_boundary,10000),]
...
school.elementary.point <- school.elementary %>% 
  ...
  st_sf() %>% 
  dplyr::select(Name)
school.elementary.point <- school.elementary.point[st_buffer(county_boundary,5000),]
```
