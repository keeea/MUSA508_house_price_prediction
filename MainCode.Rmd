...
st_drop_geometry(sale.final) %>% 
  ...
  dplyr::select(price, trailheadCount, crime_nn5, hsch_distance, parkCount) %>%
...
ggplot() +
  ...
  geom_sf(data = sale.final, aes(color = q5(price))) +
  scale_color_viridis(discrete = TRUE,
                    labels = qBr(sale.final, "price"),
                    name = "Sale Price\n(Quintile Breaks)") +
...
ggplot() +
  ...
  scale_color_viridis(discrete = TRUE,
                    labels = trailheads_boulder, "Trail Jurisdiction",
...
ggplot() +
  geom_sf(data = school.high, fill = "grey40") +
  geom_sf(data = countyParks, color = "gray15", fill = "green4") +
  scale_color_viridis(discrete = TRUE) +
  labs(title = "Parks in Boulder County, CO") +
  mapTheme()
```
...
reg.test1 <-
  sale.test %>%
  mutate(price.Predict = predict(reg.training.1, sale.test),
         price.Error = price.Predict - price,
         price.AbsError = abs(price.Predict - price),
         price.APE = (abs(price.Predict - price)) / price.Predict)

error.table1 <- data.frame(MAE = mean(reg.test6$price.AbsError, na.rm = T), MAPE = mean(reg.test6$price.APE, na.rm = T))
...
reg.test2 <-
  sale.test %>%
  mutate(price.Predict = exp(predict(reg.training.2, sale.test))-1,
         price.Error = price.Predict - price,
         price.AbsError = abs(price.Predict - price),
         price.APE = (abs(price.Predict - price)) / price.Predict)

error.table2 <- data.frame(MAE = mean(reg.test6$price.AbsError, na.rm = T), MAPE = mean(reg.test6$price.APE, na.rm = T))
...
error.table5 <- data.frame(MAE = mean(reg.test6$price.AbsError, na.rm = T), MAPE = mean(reg.test6$price.APE, na.rm = T))
...
#Polished Table of MAE & MAPE
error.table <- data.frame(MAE = mean(reg.test6$price.AbsError, na.rm = T), MAPE = mean(reg.test6$price.APE, na.rm = T))
...
reg.test6 %>%
    ...
    stat_smooth(aes(price, price),
                method = "lm", se = FALSE, size = 1, colour=palette6[1]) +
    stat_smooth(aes(price.Predict, price), 
                method = "lm", se = FALSE, size = 1, colour=palette6[2]) +
...
#Map of Dependent Variable - Sale Price
ggplot() +
  geom_sf(data = census, fill = "grey80", size=.3) +
...
reg.test6 %>% 
  ...
  stat_smooth(aes(lagPriceError, price.Error), 
             method = "lm", se = FALSE, size = 1, colour=palette5[12]) + 
...
ggplot(as.data.frame(moranTest$res[c(1:999)]), aes(moranTest$res[c(1:999)])) +
  ...
  geom_vline(aes(xintercept = moranTest$statistic), colour = palette5[10],size=1) +
...
st_drop_geometry(bothRegressions) %>%
  ...
  left_join(census) %>%
    ...
    ggplot() + 
      ...
      geom_sf(data = bothRegressions, colour = "grey10", size = .3) +
...
reg.test6 %>% 
  ...
  stat_smooth(aes(MeanPrice,MAPE), 
            method = "lm", se = FALSE, size = 1, colour=palette5[10]) + 
